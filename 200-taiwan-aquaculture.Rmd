...
```{r}
m92<-read_csv("meta/mq-with-taiwan-aquaculture.csv")
```


```{r}
m92$Group<-factor(m92$Group, levels=c("Daly River","Roper River","Mainstream","Burdekin","FitzroyQLD","Australian Aquaculture","Tamsui River"))
ggplot(m92) +
...
```{r}
m92 %>% group_by(Group) %>% summarise(Mean=mean(Coverage), Min=min(Coverage), Max=max(Coverage), Median=median(Coverage), Count=n())
```
...
```{sh, eval=FALSE}
srun -t 5-24:00:00 -p bmh --mem=48G --nodes=1 --ntasks-per-node=1 --cpus-per-task=12 $HOME/angsd/angsd -nthreads 12 \
-minInd 85 -bam bamlists/93.bamlist -ref genome/GCF_001640805.2_TLL_Latcal_v3_genomic.fna \
