...
lrts<-lrts %>%   mutate(bonfer = p.adjust(p, method = "bonferroni")) %>% 
  ...
  group_by(Chromosome) %>%
  mutate(bonferchrom = p.adjust(p, method = "bonferroni")) %>%
...
srun -t 72:00:00 -p high --mem=32G --nodes=1 --ntasks-per-node=1 --cpus-per-task=12 $HOME/angsd/angsd -P 12 \
 ...
 srun -t 72:00:00 -p high --mem=32G --nodes=1 --ntasks-per-node=1 --cpus-per-task=12 $HOME/angsd/angsd -P 12 \
 -doAsso 1 -yBin $HOME/barra-lh/bamlists/test47alt.pheno -GL 1 -minInd 42 -minMaf 0.10 \
 -minMapQ 20 -minQ 20 \
 -doMajorMinor 1 -doMaf 1 -SNP_pval 1e-6 -rf meta/lates-lgs.txt -out outputs/101/combined-asso-alt \
 -bam $HOME/barra-lh/bamlists/test47.bamlist  > outputs/101/combined-asso-alt.out 2> outputs/101/combined-asso-alt.err &
```
...
Read in and see if this changed anything, cases and controls
(base) maccamp@farm:~/barra-lh/outputs/101$ gunzip -c combined-asso-alt.lrt0.gz | sort -k 6 -n | tail
NC_066833.1	19962410	C	A	0.117364	18.307387
NC_066848.1	4407354	G	A	0.445142	18.455663
NC_066842.1	19925800	G	A	0.233628	19.368839
NC_066856.1	16177028	G	A	0.343751	19.425269
...
```{r}
lrt<- read_tsv("outputs/101/combined/combined-asso-95.lrt0.gz") %>% mutate(log10p = -log10(dchisq(LRT, df = 1))) %>% mutate(p = dchisq(LRT, df=1)) %>%
...
from about 766K to 741K snps with 95% threshold. 
```{r}
...
ggsave("outputs/101/SSS-SFF-47-inds-all-chroms-filtered.jpeg")
```
...
```{sh, eval=FALSE}
paste \
<(bcftools query -f '[%SAMPLE\t]\n' renamed-05.vcf.gz | head -1 | tr '\t' '\n') \
<(bcftools query -f '[%GT\t]\n' renamed-05.vcf.gz | awk -v OFS="\t" '{for (i=1;i<=NF;i++) if ($i == "./.") sum[i]+=1 } END {for (i in sum) print i, sum[i] / NR }' | sort -k1,1n | cut -f 2) > missingness.stats
...
bcftools view -Ov -r NC_066856.1:2843226-3080463 outputs/101/renamed-05.vcf.gz  > outputs/101/sub24-expanded.vcf
...
#calc ld
plink --vcf sub24-expanded.vcf --r2 inter-chr --ld-window-r2 0.1 --out sub24.ldf --allow-extra-chr --double-id
```

```{r}
lc<-read.delim("outputs/101/sub24.ldf.ld",sep="",stringsAsFactors=FALSE) %>% as_tibble() %>% dplyr::arrange(R2) %>%  filter(R2 >0.3)

  ggplot(lc) +
  geom_point(aes(x=BP_A, y=BP_B, color=R2), alpha=0.5) +
  scale_color_gradient(low="khaki1", high="red") + 
  ggtitle("RoSA Lca26 linkage disequilibrium plot")+
  theme_bw() +
  theme(panel.grid = element_blank(), plot.title = element_text(hjust = 0.5)) +
  xlab("\nSNP A") +
  ylab("SNP B\n")
  
ggsave("outputs/101/rosa-ld-plot.jpeg")
```
```{r}
...
ggsave("outputs/101/top-associated-variants.pdf", width=12, height=6)
