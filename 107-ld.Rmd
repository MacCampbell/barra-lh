...
plot(x, option = "scores", i = 1, j = 3, pop = meta$Group)
...
sdf<-snps %>% left_join(labels) %>% arrange(PC, Name) %>% relocate(Name, Chrom, Pos, PC)

sdf %>% filter(Chrom=="NC_066856.1")
```
...
cat meta/lates-lgs.txt  | while read line; do bcftools view -Ob -r $line outputs/107/thinned20.vcf.gz > outputs/107/bcf/$line.bcf; done;
...
thinned 20
`./run_lostruct.R -i /Users/mac/github/barra-lh/outputs/107/bcf -t snp -s 20 -m 4 -I /Users/mac/github/barra-lh/outputs/107/bcf/sample_info.tsv -j 108
cp lostruct_results/type_snp_size_20_weights_none_jobid_108/mds_coords.csv ~/github/barra-lh/outputs/107/mds_coords-20.csv`

```{r}
mds<-read_csv("outputs/107/mds_coords.csv") # 20 snp windows, thinned 50
mds<-read_csv("outputs/107/mds_coords-20.csv") # 20 snp windows, thinned 20
