...
stats2<-read_csv("outputs/100/align2.csv", col_names = c("library-id","Aligned","Dedup","Coverage"))
...
m %>% filter(Coverage>8) %>% select(Path) %>% write_tsv("bamlists/test47.bamlist", col_names = FALSE)
m %>% filter(Coverage>8) %>% select(Pheno) %>% write_tsv("bamlists/test47.pheno", col_names = FALSE)
m %>% filter(Coverage>8) %>% select(specimen_id) %>% write_tsv("bamlists/test47.names", col_names = FALSE)
m %>% filter(Coverage>8) %>% mutate(AltPheno=ifelse(Phenotype=="SFF",0,1)) %>% select(AltPheno) %>% write_tsv("bamlists/test47alt.pheno", col_names = FALSE)
m47<-m %>% filter(Coverage>8)
meta<-m47
```
...
srun -p high -t 10:00:00 --mem=32G --nodes=1 --cpus-per-task=6  $HOME/angsd/angsd -P 6  \
-bam bamlists/test47.bamlist -r NC_066833.1 \
-ref genome/GCF_001640805.2_TLL_Latcal_v3_genomic.fna \
-minInd 42 -minMapQ 20 -minQ 20 -GL 2 -doGLF 2 \
-doMajorMinor 1 -doMaf 1 -SNP_pval 1e-6 \
-doIBS 1 -doCounts 1 -doCov 1 -makeMatrix 1 -minMaf 0.05 \
-out outputs/100/47-ibs-90-Lcal01 > outputs/100/47-ibs-90-Lcal01.out 2> outputs/100/47-ibs-90-Lcal01.err &



#including chrom of interest
srun -p high -t 10:00:00 --mem=32G --nodes=1 --cpus-per-task=6  $HOME/angsd/angsd -P 6  \
-bam bamlists/test44.bamlist -r NC_066856.1 \
-ref genome/GCF_001640805.2_TLL_Latcal_v3_genomic.fna \
-minInd 40 -minMapQ 20 -minQ 20 -GL 2 -doGLF 2 \
-doMajorMinor 1 -doMaf 1 -SNP_pval 1e-6 \
-doIBS 1 -doCounts 1 -doCov 1 -makeMatrix 1 -minMaf 0.05 \
-out outputs/100/44-ibs-90-Lcal24 > outputs/100/44-ibs-90-Lcal24.out 2> outputs/100/44-ibs-90-Lcal24.err &
```
...
```{r}
m <- as.matrix(read.table("outputs/100/47-ibs-90-Lcal01.covMat"))
eig <- eigen(m)
...
pc12<-ggplot(covs) +
  ...
  geom_text_repel(data=covs %>% filter(V1  > 0.2), aes(x=V1, y=V2, label=specimen_id)) +
  scale_fill_manual(values=c("red","blue")) +
  theme_bw() +
  theme(panel.grid=element_blank()) +
  ggtitle("PCs 1 and 2") +
