...
stats<-bind_rows(stats,stats3)
m<-left_join(m,stats) %>% mutate(Pheno=ifelse(Phenotype=="SSS",0,1)) %>% mutate(Path=paste0("data/bams/",`library-id`,".sort.flt.bam"))
...
```
```{r}
m %>% filter(Coverage>10) %>% group_by(Pheno) %>% summarize(Count=n(), MedianCov=median(Coverage), AverageCov=mean(Coverage))
